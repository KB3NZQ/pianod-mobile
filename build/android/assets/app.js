...
function getPianodLines(success1, success2, success3) {
	...
	if (total >= max_response)
		resp.push({
			"code" : 400,
			"value" : "Response was too big, assuming error."
		});
...
function newSetting(label, name) {
	...
	settingInput.addEventListener('blur', function(e) {
		Ti.App.Properties.setString(name, e.value);
		reconnect(null);
	});
	settingInput.value = Ti.App.Properties.getString(name);

	var settingRow = Titanium.UI.createView({
		top : currTop,
		height : 70
	});
...
stopButton.addEventListener('click', function(e) {
	if (currentStation)
		SendPianodRequest(200, "stop now");
});
...
function newSong(label, name) {
	...
	var songRow = Titanium.UI.createView({
		top : songTop,
		height : 40
	});
